...
if [ -f "cli/pyproject.toml" ]; then
    ...
    print_status "FAIL" "CLI pyproject.toml missing"
fi

if [ -d "cli/src/ai_code_forge" ]; then
    print_status "PASS" "CLI source structure exists"
else
    print_status "FAIL" "CLI source structure missing"
fi
